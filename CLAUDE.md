...
🎯 Что умею:
• /whatsup - саммари чата
• /whatsup 2h - саммари за 2 часа
• /judge - рассудить спор
...
#### `/whatsup` (в группе)
**Описание:** Саммаризация текущего чата
**Формат:** `/whatsup [количество|период]`
**Примеры:**
- `/whatsup` - последние 50 сообщений (дефолт)
- `/whatsup 100` - последние 100 сообщений
- `/whatsup 30m` - последние 30 минут
- `/whatsup 2h` - последние 2 часа
- `/whatsup today` - с начала дня
...
#### `/judge <текст>`
**Описание:** Рассудить спор
**Формат:** `/judge Вася говорит X, Петя говорит Y. Кто прав?`
**Логика:**
...
### 📌 ФАЗА 0: Подготовка (текущее состояние)

**Что уже есть:**
- ✅ Vercel + Supabase настроены
- ✅ Webhook от Telegram работает
- ✅ Таблица `messages` создана
- ✅ Логирование сообщений работает
- ✅ Автоочистка (4 часа → нужно изменить на 7 дней!)
- ✅ Базовый /whatsup в группах
- ✅ БЫДЛАН захардкожен в SYSTEM_PROMPT

**Что нужно исправить:**
- 🔨 Изменить автоочистку на 7 дней
- 🔨 Вынести БЫДЛАНА в БД (таблица personalities)
