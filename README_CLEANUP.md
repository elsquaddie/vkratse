# Инструкция по запуску cleanup скрипта

## Что делает скрипт
`scripts/cleanup_old_messages.py` удаляет ВСЕ сообщения старше `MESSAGE_RETENTION_DAYS` (3 дня) из БД.

## Зачем он нужен?
Автоудаление работает только при новых сообщениях в чате. Если в чате давно не писали - старые данные остаются. Этот скрипт удаляет их одноразово.

## Как запустить (на своем компьютере)

### 1. Установить зависимости

```bash
pip3 install -r requirements.txt
```

### 2. Создать .env файл с реальными значениями

Скопируй `.env.example` в `.env`:

```bash
cp .env.example .env
```

Заполни реальные значения в `.env`:

```bash
# .env
TELEGRAM_BOT_TOKEN=твой_токен_бота
ANTHROPIC_API_KEY=твой_api_key
SUPABASE_URL=https://xxx.supabase.co
SUPABASE_KEY=твой_supabase_key
MESSAGE_RETENTION_DAYS=3
```

### 3. Запустить скрипт

```bash
python3 scripts/cleanup_old_messages.py
```

### Что увидишь

```
============================================================
Starting cleanup of old messages
Retention policy: 3 days
============================================================
Deleting messages older than: 2025-11-14T16:00:00+00:00

Fetching statistics before cleanup...

Found 234 old messages across 5 chats:
  Chat -1001234567890: 120 messages
  Chat -1009876543210: 80 messages
  Chat -1005555555555: 20 messages
  ...

============================================================
Delete 234 messages? [y/N]: y

Deleting old messages...
✅ Successfully deleted 234 old messages

📊 Remaining messages in database: 50

============================================================
✅ Cleanup completed successfully!
============================================================
```

### 4. Подтвердить удаление

Скрипт спросит подтверждение перед удалением:
- Введи `y` чтобы подтвердить
- Введи `n` (или Enter) чтобы отменить

## Безопасность

- Скрипт запрашивает подтверждение перед удалением
- Показывает статистику ДО удаления
- Удаляет только старые сообщения (новые остаются)
- НЕ удаляет личности, настройки пользователей, метаданные чатов

## Когда запускать?

- **Сейчас:** чтобы очистить старые данные (с 9 ноября)
- **Потом:** по необходимости, если БД переполняется

## Нужно ли запускать регулярно?

**НЕТ!** Автоудаление работает автоматически при каждом новом сообщении.

Cleanup скрипт нужен только:
1. После изменения `MESSAGE_RETENTION_DAYS`
2. Для очистки неактивных чатов
3. Для разовой очистки после накопления старых данных
