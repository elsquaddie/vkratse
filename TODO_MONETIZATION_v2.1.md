...
- [ ] **1.1** Создать SQL-миграцию для таблицы `subscriptions`
  ```sql
...
- [ ] **1.2** Создать SQL-миграцию для таблицы `usage_limits`
  ```sql
...
- [ ] **1.3** Создать SQL-миграцию для таблицы `personality_usage`
  ```sql
...
- [ ] **1.4** Создать SQL-миграцию для таблицы `group_membership_cache`
  ```sql
...
- [ ] **1.5** Добавить поле `is_group_bonus` в таблицу `personalities`
  ```sql
  ALTER TABLE personalities ADD COLUMN is_group_bonus BOOLEAN DEFAULT FALSE;
  ```

- [ ] **1.6** Применить все миграции в Supabase

- [ ] **1.7** Обновить `config.py`: добавить `PROJECT_TELEGRAM_GROUP_ID`
  ```python
  PROJECT_TELEGRAM_GROUP_ID = int(os.getenv('PROJECT_TELEGRAM_GROUP_ID', '0'))
  ```

- [ ] **1.8** Обновить `config.py`: добавить `TIER_LIMITS`
  ```python
  TIER_LIMITS = {
      'free': {
          'messages_dm': 30,
...
- [ ] **3.1** Реализовать `check_usage_limit(user_id: int, action: str) -> dict`
  ```python
...
- [ ] **3.2** Реализовать `increment_usage(user_id: int, action: str)`

- [ ] **3.3** Добавить в `db_service.py`: `get_usage_limits(user_id: int, date: date)`

- [ ] **3.4** Добавить в `db_service.py`: `increment_usage_limit(user_id: int, action: str)`

- [ ] **3.5** Встроить проверку в `modules/direct_chat.py:handle_direct_message()`
  ```python
  # В начале функции
...
- [ ] **3.6** Встроить проверку в `modules/summaries.py` (для ЛС summary)
  ```python
...
- [ ] **3.7** Создать функцию `show_upgrade_message(update, reason: str)`
  ```python
...
- [ ] **3.8** Встроить `show_upgrade_message()` во все обработчики с лимитами
...
- [ ] **6.1** Реализовать `check_personality_limit()` в `subscription.py`
  ```python
...
- [ ] **6.2** Добавить в `db_service.py`: `get_personality_usage()`
  ```python
...
- [ ] **6.3** Добавить в `db_service.py`: `increment_personality_usage()`
  ```python
...
- [ ] **6.4** Встроить проверку в `modules/summaries.py` перед генерацией саммари
  ```python
...
- [ ] **6.5** Встроить проверку в `modules/direct_chat.py` (для чата)

- [ ] **6.6** Встроить проверку в `modules/judge.py` (для судейства)

- [ ] **6.7** Обновить `/mystatus`: добавить информацию об использовании личностей
  ```python
...
- [ ] **7.1** Реализовать `is_in_project_group()` в `subscription.py`
  ```python
...
- [ ] **7.2** Добавить в `db_service.py`: `get_group_membership_cache()`

- [ ] **7.3** Добавить в `db_service.py`: `update_group_membership_cache()`

- [ ] **7.4** Создать обработчик `chat_member` в `api/index.py`
  ```python
...
- [ ] **7.5** Реализовать `handle_group_membership_change()` в `subscription.py`
  ```python
...
- [ ] **8.1** Реализовать `get_custom_personality_limit()` в `subscription.py`
  ```python
...
- [ ] **8.2** Добавить в `db_service.py`: `get_active_custom_personalities_count()`
  ```python
...
- [ ] **8.3** Реализовать `can_create_custom_personality()` в `subscription.py`
  ```python
...
- [ ] **8.4** Обновить `modules/personalities.py`: встроить проверку
  ```python
...
- [ ] **8.5** Обновить `db_service.py`: добавить `is_group_bonus` при создании личности
  ```python
...
- [ ] **9.1** Добавить поле `is_blocked` в таблицу `personalities` (SQL-миграция)
  ```sql
  ALTER TABLE personalities ADD COLUMN is_blocked BOOLEAN DEFAULT FALSE;
  ```

- [ ] **9.2** Обновить `handle_group_membership_change()`: блокировать личности при выходе
  ```python
...
- [ ] **9.3** Добавить в `db_service.py`: `block_group_bonus_personalities()`
  ```python
  async def block_group_bonus_personalities(user_id: int):
      """Заблокировать бонусные личности пользователя"""
      try:
          ...
          logger.error(f"Error blocking personalities: {e}")
  ```

- [ ] **9.4** Добавить в `db_service.py`: `unblock_group_bonus_personalities()`

- [ ] **9.5** Добавить проверку `is_blocked` при выборе личности
  ```python
...
- [ ] **10.1** Зарегистрироваться в [ЮKassa](https://yookassa.ru/), получить API-ключи

- [ ] **10.2** Добавить в `config.py`: параметры ЮKassa
  ```python
  YOOKASSA_SHOP_ID = os.getenv('YOOKASSA_SHOP_ID')
  YOOKASSA_SECRET_KEY = os.getenv('YOOKASSA_SECRET_KEY')
  ```

- [ ] **10.3** Создать модуль `services/payments.py`

- [ ] **10.4** Реализовать `create_payment_link()` в `payments.py`
  ```python
...
- [ ] **10.5** Добавить кнопку "Купить Pro (Карта/QIWI)" в `/premium`
  ```python
  keyboard = [
      [InlineKeyboardButton("💳 Купить Pro (Карта)", callback_data=sign_callback_data("buy_pro_card"))],
      [InlineKeyboardButton("🎁 Donate (Tribute.to)", url=config.TRIBUTE_URL)],
      ...
  ]

  # Обработчик callback
  async def handle_buy_pro_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
      query = update.callback_query
      await query.answer()

      user_id = update.effective_user.id
      payment_url = await create_payment_link(user_id, 'pro', 30)
      ...
      await query.edit_message_text(
          "💎 Pro-подписка\n\n"
          "Цена: $2.99/месяц\n"
          "Способ оплаты: Карта, QIWI, ЮMoney\n\n"
          "После оплаты подписка активируется автоматически!",
          reply_markup=reply_markup
      )
  ```

- [ ] **10.6** Создать webhook endpoint `api/yookassa_webhook.py`
  ```python
...
- [ ] **10.7** Реализовать обработчик webhook: проверка подписи, активация подписки

- [ ] **10.8** Настроить webhook URL в личном кабинете ЮKassa
  - URL: `https://vkratse.vercel.app/api/yookassa_webhook`

- [ ] **10.9 ТЕСТ:** Провести тестовый платеж через ЮKassa
...
- [ ] **11.1** Реализовать `create_stars_invoice()` в `payments.py`
  ```python
  async def create_stars_invoice(
      user_id: int,
      stars_amount: int = 300  # ~$3
  ) -> Message:
      """Создать инвойс для оплаты через Telegram Stars"""
      title = "Pro Subscription"
      description = "30 days of Pro features"
      payload = f"pro_sub_{user_id}_{int(time.time())}"
      ...
      invoice = await bot.send_invoice(
          ...
          currency=currency,
          prices=[{"label": "Pro Subscription", "amount": stars_amount}]
      )

      return invoice
  ```

- [ ] **11.2** Добавить кнопку "Купить Pro (Stars)" в `/premium`
  ```python
  keyboard = [
      [InlineKeyboardButton("⭐ Купить Pro (Stars)", callback_data=sign_callback_data("buy_pro_stars"))],
      [InlineKeyboardButton("💳 Купить Pro (Карта)", callback_data=sign_callback_data("buy_pro_card"))],
      ...
  ]
  ```

- [ ] **11.3** Создать обработчик `PreCheckoutQuery`
  ```python
  async def handle_pre_checkout_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
      """Обработчик перед оплатой"""
      query = update.pre_checkout_query

      # Всегда подтверждаем (дополнительные проверки можно добавить)
      await query.answer(ok=True)
  ```

- [ ] **11.4** Создать обработчик `SuccessfulPayment`
  ```python
  async def handle_successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
      ...
      if success:
          ...
          await update.message.reply_text(
              ...
              "Обратитесь в поддержку: @admin_username"
          )
  ```

- [ ] **11.5 ТЕСТ:** Провести тестовую покупку через Telegram Stars
