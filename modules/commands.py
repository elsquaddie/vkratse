...
async def handle_start_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if action == "back_to_main":
            ...
            # Start judge flow directly via inline button
            text = f"""⚖️ Рассудить спор - бот поможет рассудить спор в выбранном стиле!

Нажми кнопку ниже и опиши спор в следующем сообщении.
...
Дамирка и Настька поспорили о плоской земле. Кто прав?"""
            ...
            # Inline button to start judge flow
            keyboard = [[InlineKeyboardButton("⚖️ Начать судейство", callback_data=sign_callback_data("start_judge"))]]
            reply_markup = InlineKeyboardMarkup(keyboard)
